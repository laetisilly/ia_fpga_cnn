...
def projection_block(x,out_channel,block_id):
    prefix = 'block_{}_'.format(block_id)
    x = Conv2D(filters=out_channel,kernel_size = 1,   padding='same',use_bias=False,name= prefix + 'compress')(x)
...
def MobileNetV2(input_image = (224,224,3), n_classes=1000):
    ...
    x = projection_block(x, out_channel=16,block_id=1)
