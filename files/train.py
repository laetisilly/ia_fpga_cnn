...
def train(input_height,input_width,input_chan,batchsize,learnrate,epochs,keras_hdf5,tboard):
    ...
    # Each image is 224x224x8bits
    (x_train, y_train), (x_test, y_test) = datadownload()
    print ('Dataset downloaded and pre-processed')

    model = keras.applications.mobilenet_v2.MobileNetV2(input_shape=(input_height,input_width,input_chan), alpha=1.0, include_top=True, weights='imagenet', input_tensor=None, pooling='max', classes=1000)
